...
case '6': case '7': case '8': case '9':	/*ch为数字字符*/
		...
		case '\n':
			...
			case '+':
				...
				push(pop()+right_oprand);/*左操作数出栈加右操作数，和再压栈*/
				break;
			case '-':
				right_oprand=pop();/*右操作数出栈*/
				push(pop()-right_oprand);/*左操作数出栈减右操作数，差再压栈*/
				break;
			case '*':
				right_oprand=pop();/*右操作数出栈*/
				push(pop()*right_oprand);/*左操作数出栈乘以右操作数，积再压栈*/
				break;
			case '/':
				right_oprand=pop();/*右操作数出栈*/
				if(right_oprand)/*除数为零吗*/
					push(pop()/right_oprand);/*除数非零，左操作数出栈除以右操作数，商再压栈*/
				else
				{
